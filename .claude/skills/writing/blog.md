...
## 트리거
- `/blog` → 블로그 글쓰기 세션 시작
- `/blog [주제]` → 해당 주제로 바로 시작
- `/blog review` → 기존 초안 리뷰 및 개선
...
- **핵심 기술 용어는 영어 원문을 병기한다.** 한국어 문장 속 영어 단어는 볼드처럼 시각적으로 부각되는 효과가 있다. 이를 의도적으로 활용한다.
...
- **핵심 주장은 짧은 문장으로 끊는다.** 전후의 맥락은 좀 더 길게 쓴다.
  - 예: "프로듀서가 메시지를 보낸다. 이때 acks 설정에 따라 리더 브로커는 ISR 전체가 복제를 완료할 때까지 기다리거나, 리더 혼자 기록한 직후 즉시 응답을 돌려보낸다. 이 차이가 성능과 안정성의 트레이드오프를 결정한다."
...
3. /study 세션에서 나온 내용이 있으면 Phase 2~3 대화 내용을 활용한다.
...
`/study`로 학습한 내용을 블로그로 쓰고 싶을 때 `/blog`를 별도 호출한다.
`/study`의 Phase 4에서 저장된 학습 메모(`study-notes/`)를 소재로 활용할 수 있다.
- `/blog`에 주제를 주면 관련 학습 메모가 있는지 `study-notes/`를 먼저 확인한다.
