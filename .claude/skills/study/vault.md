...
## 트리거
- `/study-vault` → 책 선택부터 시작
- `/study-vault [책이름 키워드]` → 해당 책 바로 시작
- `/study-vault [책이름] [챕터범위]` → 특정 챕터만 (예: `/study-vault 카프카 Ch3-Ch5`)
...
| **적합한 상황** | 핵심 챕터 깊이 파기 | 새 책 전체 조감, 시험 준비, 복습용 노트 |

두 스킬은 보완적이다:
1. `/study-vault`로 책 전체 구조를 먼저 파악한다.
2. `/study`로 핵심 챕터를 깊이 파고든다.
3. vault의 연습문제로 복습한다.
...
3. 사용자에게 토픽 계층구조를 보여주고, 범위를 확인한다.
   ...
   - 설치/환경구축 같은 챕터는 `/lab` 스킬로 대체 가능하므로 스킵 제안
...
├── 00-exam-traps.md             # 자주 혼동하는 개념, 함정
├── 01-[토픽명]/
│   ├── concepts.md              # 개념 노트 (이 토픽의 모든 핵심 개념)
│   └── practice.md              # 연습 문제 (8문제 이상)
├── 02-[토픽명]/
│   ├── concepts.md
│   └── practice.md
└── ...
...
규칙:
- 각 개념은 **정의 → 왜 필요한가 → 동작 원리 → 관련 개념** 순서
- 비교가 의미 있는 경우 비교 테이블 포함
- ASCII 다이어그램은 구조/흐름 이해에 도움이 될 때만 사용
...
- 오답 보기(선택형의 경우)는 실제 도메인 개념이어야 한다 (명백한 오답 금지)
...
3. exam-traps에서 관련 concepts.md로 링크한다.
...
- [ ] 비교 테이블이 필요한 곳에 포함되어 있는가?

### 연결 검수
- [ ] 모든 상호 참조 링크가 실제 파일/앵커를 가리키는가?
- [ ] dashboard에서 모든 토픽으로 링크가 되어 있는가?
- [ ] exam-traps의 혼동 개념이 해당 concepts.md로 연결되어 있는가?
