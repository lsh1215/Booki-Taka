...
## 트리거
사용자가 `/setup-quiz`를 입력하면 이 스킬이 활성화된다.
...
- App 이름: `BookiTaka Quiz Bot` (또는 원하는 이름)
...
- 봇을 채널에 초대: `/invite @BookiTaka Quiz Bot`
...
2. 스레드에 정답을 spoiler 형태로 리플라이 (답변 작성 후 확인용):
   "||정답: {answer}||" → Slack은 spoiler 미지원이므로 대신:
   접힌 형태로: ":white_check_mark: *정답 확인* (답변 작성 후 펼치세요)\n>{answer}"
   → 실제로는 바로 보이므로, 정답은 별도 리플라이로 1분 뒤 발송하거나,
     사용자가 이모지(👀)를 달면 발송하는 방식 고려.

   간단한 구현: 정답은 다음 날 채점 시 함께 공개.
"""
```
...
study.md 스킬의 Phase 3 (검증) 단계에서, 검증에 사용한 질문을 자동으로 quiz_bank.json에 추가하는 로직을 포함한다.
...
- quiz_bank.json이 비어있으면 "아직 등록된 퀴즈가 없습니다. /study로 학습을 시작하세요!" 메시지를 보낸다.
