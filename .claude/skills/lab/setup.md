...
## 트리거
- `/lab` → 기술 선택부터 시작
- `/lab kafka` → 카프카 lab 구축
- `/lab es` → Elasticsearch lab 구축
- `/lab db` → MySQL lab 구축
- `/lab redis` → Redis lab 구축
- `/lab [기술] destroy` → 해당 lab 정리 (docker compose down -v)
...
1. 사용자가 `/lab [기술]` 호출 (또는 `/study` 세션 중 실습 요청)
2. 현재 학습 주제를 파악한다.
...
- **포트 충돌**: 여러 lab을 동시에 띄울 수 있으므로 포트가 겹치지 않게 관리한다.
  - 각 lab의 포트 매핑을 기본값에서 오프셋을 두어 분리
